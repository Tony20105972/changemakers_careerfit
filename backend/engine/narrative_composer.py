"""Deterministic narrative composer for CareerFit V1.

The composer owns narrative generation. It reads completed engine outputs and
the current Skill Intelligence Registry, then returns the six narrative outputs
used by the Report JSON builder. It does not score, match, extract evidence, or
call an LLM.
"""
...
def load_skill_registry(path: str | Path | None = None) -> dict[str, dict[str, Any]]:
...
def compose_report_narratives(
    *,
    ...
    """Compose all V1 narrative outputs from completed engine outputs."""
    skill_registry = registry if registry is not None else load_skill_registry(registry_path)
    meta = _as_dict(meta)
    scores = _as_dict(scores)
    ordered_strengths = _ordered_items(strengths)
    ordered_gaps = _ordered_items(gaps)
    evidences = _flatten_evidence_mapping(evidence_mapping)

    confidence_level = str(meta.get("confidence_level") or "").upper()
    is_low_confidence = confidence_level == "LOW"
    profile_label = _profile_label(str(meta.get("primary_profile") or ""))
    score = _score_value(scores)
    fit_level = str(meta.get("fit_level") or scores.get("fit_level") or "")

    strength_narratives = [
        build_strength_narrative(item, skill_registry, evidences, is_low_confidence)
        for item in ordered_strengths
    ]
    gap_narratives = [
        build_gap_narrative(item, skill_registry, evidences, is_low_confidence)
    ...
    output = {
        ...
        "skill_explanations": [
            build_skill_explanation(source, item, skill_registry, evidences, is_low_confidence)
            for source, item in _skill_explanation_inputs(ordered_strengths, ordered_gaps)
        ],
        "job_outlook": build_job_outlook(
            profile_label=profile_label,
            score=score,
            scores=scores,
            strengths=ordered_strengths,
...
def build_strength_narrative(
    item: dict[str, Any],
    registry: dict[str, dict[str, Any]],
    evidence_mapping: list[dict[str, Any]],
    is_low_confidence: bool = False,
) -> dict[str, Any]:
    """Build one strength narrative from a selected strength item."""
    ...
    narrative = f"{description['strength_narrative']} {description['business_value']} {evidence_text}{tone}"
    return {
        "skill_key": skill_key,
...
def build_gap_narrative(
    ...
    """Build one gap narrative from a selected gap item."""
    skill_key = _skill_key(item)
    description = _description_for(skill_key, item, registry)
    evidence_text = _evidence_summary(item, evidence_mapping)
    ...
    narrative = f"{description['gap_narrative']} {description['market_context']} {evidence_text}{tone}"
    return {
        "skill_key": skill_key,
        "label_ko": description["label_ko"],
        "rank": _rank_value(item),
...
def build_skill_explanation(
    ...
    """Build one Report JSON skill explanation item."""
    skill_key = _skill_key(item)
    description = _description_for(skill_key, item, registry)
    evidence_text = _evidence_summary(item, evidence_mapping)
...
def build_market_outlook(
    ...
    """Build a compact market-context phrase from top strength and gap skills."""
    strength_text = _join_korean(_labels_for_items(strengths[:2], registry)) or "확인된 강점"
    gap_text = _join_korean(_labels_for_items(gaps[:2], registry)) or "추가 검증 영역"
    return f"{strength_text}은 시장 맥락에서 강점 신호로 읽히며, {gap_text}은 설명 공백으로 함께 검토됩니다."


def build_job_outlook(
    profile_label: str,
    score: float,
    scores: dict[str, Any],
    strengths: list[dict[str, Any]],
    ...
    """Build the job outlook narrative."""
...
def build_final_assessment(
    ...
    """Build the final assessment narrative."""
    strength_text = _join_korean(_labels_for_items(strengths[:2], registry)) or "확인된 강점"
...
def _skill_explanation_inputs(
    strengths: list[dict[str, Any]], gaps: list[dict[str, Any]]
    ...
    for source, items in (("strength", strengths), ("gap", gaps)):
        for item in items:
            ...
            if not skill_key or skill_key in seen:
                continue
            seen.append(skill_key)
            inputs.append((source, item))
    return inputs


def _flatten_evidence_mapping(evidence_mapping: list[dict[str, Any]]) -> list[dict[str, Any]]:
    flattened: list[dict[str, Any]] = []
    for group in _as_list(evidence_mapping):
        group_dict = _as_dict(group)
        nested = _as_list(group_dict.get("evidences"))
        if nested:
            for evidence in nested:
                evidence_dict = dict(_as_dict(evidence))
                if not evidence_dict.get("skill_key"):
                    evidence_dict["skill_key"] = group_dict.get("skill_key")
                flattened.append(evidence_dict)
        else:
            flattened.append(group_dict)
    return sorted(flattened, key=lambda item: (_skill_key(item), _evidence_sort_key(item), _clean_text(item.get("original_text"))))
...
def _evidence_for_item(item: dict[str, Any], evidence_mapping: list[dict[str, Any]]) -> list[dict[str, Any]]:
    evidence_ids = _unique_sorted_strings(_as_list(item.get("evidence_ids")))
    if evidence_ids:
        matched = []
        for evidence in evidence_mapping:
            if _evidence_sort_key(evidence) in evidence_ids:
                matched.append(evidence)
        if matched:
            return matched
    skill_key = _skill_key(item)
    return [evidence for evidence in evidence_mapping if _skill_key(evidence) == skill_key]


def _description_for(skill_key: str, item: dict[str, Any], registry: dict[str, dict[str, Any]]) -> dict[str, Any]:
    description = dict(registry.get(skill_key) or {})
    label = str(item.get("label_ko") or description.get("label_ko") or skill_key or "스킬")
    fallback = _fallback_description(label)
...
def _fallback_description(label: str) -> dict[str, Any]:
    return {
        "label_ko": label,
        ...
        "workplace_behavior": f"실무에서는 {label} 관련 회의, 문서, 고객 접점, 내부 협업 과정에서 드러납니다.",
        "business_value": f"조직 안에서 {label} 역량은 실행 기준을 명확하게 만들고 협업 비용을 줄입니다.",
        "career_relevance": f"{label} 경험은 목표 직무 적합도를 설명하는 보조 근거가 됩니다.",
        "market_context": f"채용시장에서는 {label} 역량을 실제 업무 장면에서 어떻게 사용했는지를 중요하게 봅니다.",
        "strength_narrative": f"당신의 경험에서 {label} 역량이 강점으로 나타난다면, 이는 관련 업무 장면을 실제로 다루어본 근거가 있다는 의미입니다.",
        "gap_narrative": f"{label} 역량이 부족한 영역으로 나타난다면, 이는 경험 서술에서 관련 장면이 충분히 드러나지 않았다는 의미일 수 있습니다.",
        "development_direction": f"{label} 역량은 관련 상황, 본인의 역할, 판단 기준, 결과를 함께 설명할 때 더 선명하게 드러납니다.",
...
def _assert_no_banned_text(value: Any) -> None:
    banned_terms = ("expected_score_gain", "difficulty", "time_estimate", "{", "}", "TODO", "TBD")
...
def _ordered_items(items: list[dict[str, Any]]) -> list[dict[str, Any]]:
    return sorted([_as_dict(item) for item in _as_list(items)], key=lambda item: (_rank_value(item), _skill_key(item)))
...
def _evidence_sort_key(item: dict[str, Any]) -> str:
    return str(item.get("evidence_id") or item.get("id") or item.get("career_history_id") or "").strip()
